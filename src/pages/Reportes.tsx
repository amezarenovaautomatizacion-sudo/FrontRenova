...
const loadTareas = async (proyectoId: number, filtros?: any) => {
  ...
  try {
    ...
    setError(error.response?.data?.message || 'Error cargando colaboradores');
  } finally {
  ...
  const loadEmpleadosParaAsignar = async (filtros?: Partial<FiltrosEmpleados>, modo: 'supervisados' | 'todos' = 'supervisados') => {
    ...
    try {
      ...
      setError(error.response?.data?.message || 'Error cargando colaboradores disponibles');
    } finally {
...
useEffect(() => {
  ...
  const handleAsignarEmpleado = async (empleadoId: number) => {
    ...
    try {
      ...
      if (response.data.success) {
        setSuccess('Colaborador asignado al proyecto exitosamente');
      ...
      setError(error.response?.data?.message || 'Error asignando colaborador');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleQuitarEmpleado = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setSuccess('Colaborador removido del proyecto exitosamente');
      ...
      setError(error.response?.data?.message || 'Error removiendo colaborador');
    } finally {
      setLoading(false);
    }
  };
...
const openProyecto = async (proyecto: Proyecto) => {
  ...
  const getRolBadge = (rol: string) => {
    const config: Record<string, { bg: string; icon: any; label: string }> = {
      ...
      manager: { bg: 'warning', icon: faUserTie, label: 'GERENTE' },
      employee: { bg: 'info', icon: faUser, label: 'COLABORADOR' }
    };
        ...
        <Card className="shadow-sm border-0">
          <Card.Body className="text-center py-5">
            ...
            <Badge bg="secondary" className="fs-6 p-2">
              ...
              Rol: {userRol === 'admin' ? 'ADMINISTRADOR' : userRol === 'manager' ? 'GERENTE' : 'COLABORADOR'}
            </Badge>
          </Card.Body>
        </Card>
      </Container>
      ...
      >
        ...
        <Modal.Body>
          ...
          {loadingJefe ? (
            <div className="text-center py-4">
              <Spinner animation="border" variant="primary" />
              <p className="mt-2 text-muted">Cargando colaboradores...</p>
            </div>
            ...
            <Card className="text-center py-5 border-0 bg-light">
              <Card.Body>
                ...
                <h6 className="text-muted">No se encontraron colaboradores</h6>
            ...
            <>
              <p className="text-muted small mb-3 d-flex align-items-center">
                ...
                Mostrando {empleadosDisponibles.length} colaboradores
              </p>
      ...
      >
        ...
        <Modal.Body>
          ...
          {loadingAsignar ? (
            <div className="text-center py-4">
              <Spinner animation="border" variant="primary" />
              <p className="mt-2 text-muted">Cargando colaboradores disponibles...</p>
            </div>
          ) : empleadosParaAsignar.length === 0 ? (
            <Card className="text-center py-5 border-0 bg-light">
              <Card.Body>
                <FontAwesomeIcon icon={faUsers} size="3x" className="text-muted mb-3 opacity-50" />
                <h6 className="text-muted">No hay colaboradores disponibles para asignar</h6>
                <p className="text-muted mb-0">
                  {modoSeleccion === 'supervisados' 
                    ? 'No tienes subordinados disponibles o ya están asignados'
                    : 'No hay colaboradores disponibles en la empresa'}
                </p>
              </Card.Body>
            </Card>
          ) : (
            <>
              <p className="text-muted small mb-3 d-flex align-items-center">
                <FontAwesomeIcon icon={faInfoCircle} className="me-2" />
                Mostrando {empleadosParaAsignar.length} colaboradores disponibles
              </p>
              <ListGroup variant="flush" className="border rounded">
          ...
          {selectedEmpleado && (
            <>
              ...
              <Alert variant="warning" className="mb-0">
                ...
                Este colaborador tiene <strong>{selectedEmpleado.TareasActivas}</strong> tareas activas asignadas. 
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Text className="text-muted">
                    Debe ser un colaborador con rol de administrador o gerente
                  </Form.Text>
                </Form.Group>
              </Col>
              <Col md={6}>
      ...
      >
        ...
        <Modal.Body>
          <Form>
            ...
            {!isEditingTarea && (
              <Form.Group className="mb-3">
                <Form.Label className="fw-bold">Asignar a colaborador (opcional)</Form.Label>
                <Form.Select
